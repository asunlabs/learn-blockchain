...
> We fill in the body of the deref method with &self.0 so deref returns a reference to the value we want to access with the _ operator. The main function in Listing 15-9 that calls _ on the MyBox<T> value now compiles, and the assertions pass!
...
> Note that the _ operator is replaced with a call to the deref method and then a call to the _ operator just once, each time we use a _ in our code. Because the substitution of the _ operator does not recurse infinitely, we end up with data of type i32, which matches the 5 in assert_eq! in Listing 15-9.
